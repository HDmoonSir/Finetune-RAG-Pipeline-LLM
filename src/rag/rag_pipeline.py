...
def load_llm(
    ...
    if unsupervised_lora_path and model is not None:
        print(f"Loading and merging unsupervised LoRA adapter from {unsupervised_lora_path}...")
        try:
            model = PeftModel.from_pretrained(model, unsupervised_lora_path)
            model = model.merge_and_unload()
            print("Unsupervised LoRA adapter loaded and merged.")
        except Exception as e:
            print(f"Error loading unsupervised LoRA adapter from {unsupervised_lora_path}: {e}")
            return None
...
def setup_retriever(
    ...
    if knowledge_base_path == "default":
        print(f"Loading default KorQuAD dataset ({default_knowledge_base_dataset}) for RAG knowledge base...")
...
def main(
    ...
    try:
        ...
        llm_pipeline = load_llm(
            ...
            model_id=model_id,
            unsupervised_lora_path=unsupervised_lora_path,
