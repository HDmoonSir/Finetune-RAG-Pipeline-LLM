...
def load_llm(model_type: str, lora_path: str = None, model_id: str = config.LOCAL_MODEL_ID):
...
def main(model_type: str, lora_path: str = None, knowledge_base: str = "default", model_id: str = config.LOCAL_MODEL_ID):
    """메인 실행 함수"""
    try:
        # 1. 모델 로드 (한 번만 실행)
        llm = load_llm(model_type, lora_path, model_id)
